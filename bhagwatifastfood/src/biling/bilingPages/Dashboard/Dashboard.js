...
import { Button, Divider, FilledInput, FormControlLabel, FormHelperText, FormLabel, Input, InputAdornment, Paper, Radio, RadioGroup, Switch } from '@mui/material';
...
function Dashboard() {
    ...
    const getAllItems = async (menu) => {
        ...
        try {
            ...
            const response = await axios.get(`${BACKEND_BASE_URL}menuItemrouter/getItemData?menuId=${menu}`, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            });
    ...
    const addVariantFields = () => {
        ...
        if (editData) {
            setUpdatedVariantsName(prevState => {
                if (unit.unit === 'No') {
                    return [{ unit: unit.unit, price: unit.price, status: true }, ...prevState];
                } else {
                    return [...prevState, { unit: unit.unit, price: unit.price, status: true }];
                }
            });
        } else {
    ...
    const getSubCategory = async (menu) => {
        ...
        try {
            const token = localStorage.getItem('token');
            const response = await axios.get(`${BACKEND_BASE_URL}menuItemrouter/ddlSubCategory?menuId=${menu}`, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            });
    ...
    const handleSubCategoryClick = async (subCategoryId, menuCategory) => {
        ...
        try {
            ...
            await axios.get(`${BACKEND_BASE_URL}menuItemrouter/getItemData?menuId=${menuCategory}&subCategoryId=${subCategoryId}`, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            })
    ...
    const handleItemStatusChange = async (item) => {
        ...
        if (userEnteredPassword === correctPassword) {
            ...
            await axios.get(`${BACKEND_BASE_URL}menuItemrouter/updateItemStatus?menuId=${menuId}&itemId=${item?.itemId}&status=${switchValue}`, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            })
    ...
    const handleSubCategoryStatusChange = async () => {
        ...
        if (userEnteredPassword === correctPassword) {
            ...
            await axios.get(`${BACKEND_BASE_URL}menuItemrouter/updateItemStatus?menuId=${menuId}&subCategoryId=${finalSelected}&status=${switchValue}`, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            })
    ...
    const [showNext, setShowNext] = useState(false); // Initialize as false initially
                    ...
                    <div className='h-full grid grid-cols-12'>
                        <div className={`h-full ${editPriceMode ? 'col-span-9' : 'col-span-10'}`}>
                            <div className='relative h-full'>
                                ...
                                <div className='flex ml-12 gap-3 menuCategoryScroll overflow-x-auto h-full' style={{ whiteSpace: 'nowrap' }} ref={scrollContainerRef}>
                        ...
                        {
                            !editPriceMode ? (
                                <div className='flex gap-4 col-span-2 justify-self-end w-3/5 pr-3 h-full'>
                    ...
                    <div className="sidebar overflow-y-auto blackCountLogoWrp shadow-2xl py-4 my-4 mr-4 static">
                        {subCategories.map(subcategory => (
                            <div
                                ...
                                className={`sidebar_menu my-2 flex jystify-between cursor-pointer rounded-lg p-2 text-start text-lg ${clickedSubCategory === subcategory.subCategoryId ? 'ClickedBlueBg' : ' hover:bg-gray-700 hover:text-white'}`}
                                onClick={() => handleSubCategoryClick(subcategory.subCategoryId, menuId)}
                            >
                    ...
                    <div className="grid grid-cols-12 gap-4">
                        ...
                        <div className="col-span-2">
                            <TextField
                                ...
                                helperText={allFormValidation.itemShortKey ? 'Item Short Code is required' : ''}
                                label="Short Code"
                                variant="outlined"
                                autoComplete="off"
                        ...
                        <div className="col-span-2">
                            <TextField
                                ...
                                helperText={allFormValidation.price ? 'Item Price is required' : ''}
                                label="Price"
                                variant="outlined"
                                    ...
                                    if (regex.test(newValue)) {
                                        if (editData) {
                                            setEditData(prevState => ({
                                                ...prevState,
                                                variantsList: prevState.variantsList.map((variant, index) =>
                                                    index === 0 ? { ...variant, price: newValue } : variant
                                                )
                                            }));
                    ...
                    <div className="flex w-full gap-6 ">
                        ...
                        <div className='customVariantsHeight'>
                            {variantFields?.map((period, index) => (
                                <div key={index} className="flex w-full mb-6 mt-2 px-2 pr-4  gap-6 items-center">
                                    ...
                                    <div className='w-full'>
                                        <TextField
                                            ...
                                            onChange={(e) => {
                                                ...
                                                if (period?.unit === 'NO') {
                                                    if (editData) {
                                                        const updatedVariantsList = [...editData.variantsList];
                                    ...
                                    <div className='w-full'>
                                        <TextField
                                            ...
                                            onChange={(e) => {
                                                ...
                                                const regex = /^[0-9]*\.?[0-9]*$/;
                                                if (regex.test(value)) {
                                                    setSecondVariantData({ ...secondVariantData, price: value });
                                                }
                                            }}
                                        />
                                    </div>
                                    <div className='w-full'>
                                        <button onClick={handleSecondAddVarinats} className='w-full addCategorySaveBtn'>Add</button>
                                    </div>
                                </div>
                                <hr className="my-6" />
                            </div>
                        ...
                        <div>
                            ...
                            <div className="w-full">
                                {variantMode.isView && (
                                    <div className="flex gap-6 mt-6 w-full">
                                        <div className="w-full">
                                            <button onClick={() => handleManualVariantsPopUp()} className="addCategorySaveBtn ">Edit</button>
                                        </div>
                                        <div className="w-full">
                                            <button onClick={handleClose} className="addCategoryCancleBtn  bg-gray-700">Cancel</button>
                                        </div>
